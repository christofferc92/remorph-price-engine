...
- `estimate_quality` still reflects the estimator’s confidence, but `estimate_range`/`labor_range`/`material_range` now come from `computeRangeFromLineItems` instead of the old low/mid/high sweep (`contract.ts:553-760`). The backend uses the single “mid” estimate, computes a weighted uncertainty percentage (trade-group subtotals × default pct per trade group), then tightens that pct when room measurements or site conditions are confirmed and widens it slightly when blocking `needs_confirmation_ids` exist. The final `low/high` totals stay at least ±max(2 000 SEK, 3 % of mid) to avoid collapsing while still growing with the overall scope.
...
- `estimate_range` (low/mid/high totals) plus the labor/material swings computed in `runEstimateFromNormalized`. The range now adapts via weighted uncertainty pct per trade group, shrinks when room measurements/site_conditions are confirmed, widens when `needs_confirmation_ids` remain, and still enforces a minimum ±max(2 000 SEK, 3 % of mid) so the UI always renders a live interval.
